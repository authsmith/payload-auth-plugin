...
import { UserNotFoundAPIError, MissingCollection, } from "../../errors/apiErrors";
import { createSessionCookies, invalidateOAuthCookies, } from "../../utils/cookies";
import { removeExpiredSessions } from "../../utils/session";
export async function OAuthAuthentication(pluginType, collections, allowOAuthAutoSignUp, useAdmin, secret, request, successRedirectPath, errorRedirectPath, userData) {
    const { email: _email, sub, name, scope, issuer, picture, access_token, } = userData;
    ...
    if (cookieState) {
        try {
            ...
            successRedirectionURL.searchParams.set("state", encodeURIComponent(cookieState));
            console.log("Added state to redirect URL:", cookieState);
        }
        catch (error) {
            console.error("Failed to parse state data, continuing without state:", error);
        }
    }
...
//# sourceMappingURL=oauth_authentication.js.map